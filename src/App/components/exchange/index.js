...
function Exchange ({ toGo, step }) {
  ...
  const { fromAmount, fromAccount, accountsList, coinsList, toCoin, toAddress, bestRate, isFindBestRate, isLoading, partner } = useSelector(state => state.exchange.exchangeInfo)
  let loaded = (isLoading >= 253) ? true : false
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.item}>
          ...
          {loaded && isFindBestRate ? `${bestRate} ${toCoin.ticker}` : <span className={styles.skeleton}></span>}
          </span>
        </div>
      </div>
      ...
      {/* <p style={{ maxWidth: 600, overflowWrap: 'break-word' }}>
        ...
        {` ${bestRate} ${loaded} ${isFindBestRate}`}
      </p> */}

      <div className={styles.container}>
        <button 
          disabled={!isFindBestRate}
          onClick={
            () => toGo(nextStep, true)
          }>Exchange</button>
      </div>
