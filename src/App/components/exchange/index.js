...
function Exchange ({ toGo, step }) {
  ...
  const { fromAmount, fromAccount, accountsList, coinsList, toCoin, toAddress, bestRate, isFindBestRate, isLoading } = useSelector(state => state.exchange.exchangeInfo)
  let loaded = (isLoading === 253 || isLoading >= 506) ? true : false
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.item}>
          ...
          {isFindBestRate ? `${bestRate} ${toCoin.ticker}` : <span className={styles.skeleton}></span>}
          </span>
        </div>
      </div>
